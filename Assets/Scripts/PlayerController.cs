...
public class PlayerController : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        else if (Physics.Raycast(rb.transform.position, Vector3.down, 1 + 0.001f))
        {
            isJumping = false;
        }
        Look();
    ...
    {
        Movement();
        if (isSprinting == true && !isCrouching && !isJumping)
        {
            rb.velocity = new Vector3(rb.velocity.x * sprintingMultiplier, rb.velocity.y, rb.velocity.z * sprintingMultiplier);
        }
    }
